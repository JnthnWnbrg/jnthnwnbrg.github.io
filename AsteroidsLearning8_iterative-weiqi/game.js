...
var shouldDrawImages = true; //true; 
...
var Ship = function(json){
	...
	this.width = 30;
	this.height = 30;
	this.x = game.width/2 - this.width/2 + Math.random()*this.width*6;
...
Ship.prototype.getSensorDistances = function(){
	...
	for(var ii in game.ships){
		var distance = Math.sqrt( Math.pow(game.ships[ii].x + game.ships[ii].width/2 - this.x + this.width/2, 2) + Math.pow(game.ships[ii].y + game.ships[ii].height/2 - this.y + this.height/2, 2));
		
		/*
		var diagonal = Math.sqrt(
		  Math.pow(game.ships[ii].width, 2) + 
		  Math.pow(game.ships[ii].height, 2)
		  )/2;
		  */
		var extra = game.ships[ii].width+game.ships[ii].height;
		if(distance <= maxSensorSize + extra){
			for(var j = 0; j < nbSensors; j++){
				...
				var objx = game.ships[ii].x + game.ships[ii].width/2;
				var objy = game.ships[ii].y + game.ships[ii].height/2;


				if(true || Math.abs(Math.atan2(objy - y1, objx - x1) - Math.atan2(y2 - y1, x2 - x1)) <= Math.PI * 2 / nbSensors){
					var d = collisionSegmentAABB(x1, y1, x2, y2, game.ships[ii].x, game.ships[ii].y, game.ships[ii].width, game.ships[ii].height);
...
Ship.prototype.isDead = function(){
  ...
  if(!this.alive) {
	...
	for(var i in game.ships2){
		if(collisionAABB(this, game.ships2[i])){
			this.hp--;
		}
	}
	this.hp--;
...
Game.prototype.gameStart = function(){
  ...
  this.boards=[];
	...
	for(var i in this.gen){
		var s = new Ship();
		s.x = i*this.width/13 ; //game width /population
	...
	for (var i = 0; i< numBoards;i++) {
	  this.boards.push([["empty","empty"],["empty","empty"]]);
	}
...
Game.prototype.display = function(){
  ...
  if(shouldShowText) {
    this.ctx.fillStyle = "white";
  	...
  	this.ctx.fillText("boards: ", 10, 200);
  	for (var i in this.boards) {
  	  for (var row in this.boards[i]) {
  	    var str = "";
  	    for (var column in this.boards[i][row]) {
  	      str+=this.boards[i][row][column]+"|";
  	    }
  	    this.ctx.fillText(str, 10, 225+75*i+25*row);
  	    
  	    str="";
  	    for (var column in this.boards[i][row]) {
  	      str+="-"; //divider
  	    }
  	    this.ctx.fillText("------------------", 10, 225+25*i+25*(row+0.5));
	...
	var start = function(){
	  //44//console.log("var start = function()");
		...
		Neuvol2 = new Neuroevolution_ReLU ({
			population: numBoards, //previously 50
			network:[nbSensors, [9], 2],
			randomBehaviour:0.1,
			mutationRate:0.5,
			mutationRange:2,
