var Neuroevolution = function(options){
	...
	*/
		...
		for(var nb = 0; nb < nbChilds; nb++){
			...
			for(var i2 in data.network.weights){
				if(Math.random() <= self.options.mutationRate){
					data.network.weights[i2] += Math.random() * self.options.mutationRange * 2 - self.options.mutationRange;
				}
			}
			datas.push(data);
		}
	...
	Generation.prototype.generateNextGeneration = function(){
		...
		for(var i2 = 0; i2 < Math.round(self.options.randomBehaviour * self.options.population); i2++){
